  ...
  useEffect(() => {
    const fetchPublicList = async () => {
      try {
        ...
        setError('Failed to load the shopping list. The link may be invalid.');
        console.error(err);
      } finally {
        setLoading(false);
      }
    ...
    <Container>
      <div className="max-w-3xl mx-auto py-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold">Shopping list by <span className="text-orange-500">{ownerName}</span></h1>
        </div>

        {list.length === 0 ? (
          <div className="text-center py-12 bg-gray-50 rounded-lg">
            <p className="text-gray-500">This shopping list is empty.</p>
          </div>
        ) : (
